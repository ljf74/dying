...
const putStart = (canvas, x, y, radius = 1) => {
    const COLOR1 = [255];
...
export const createBackgroundBitmap = (assets, width, height, yoff = 0) => {
    return generateFreeStyleBitmap(assets, width, height, (canvas) => {
        const MOON_XOFF = -32;
        const MOON_YOFF = 32;
        const MOON_DIAM = 44;
        const MOUNTAINS_HFACTOR = 12;
        const MOUNTAINS_SHIFTX = 0;
        const MOUNTAIN_CURVE = (t) => {
            let s = t / width;
        ...
        for (let x = 0; x < width; ++x) {
            y = height - MOUNTAIN_YOFF +
                Math.round(MOUNTAIN_CURVE(x + MOUNTAINS_SHIFTX) * MOUNTAINS_HFACTOR);
