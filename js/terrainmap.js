...
const getTile = (data, width, height, x, y, def = 1) => {
    if (x < 0 || y < 0 || x >= width || y >= height)
        return def;
    return data[y * width + x];
};
const computeTerrainDataForGroundTile = (data, width, height, out, x, y) => {
...
export const createTerrainMap = (data, width, height) => {
    let out = (new Array(width * height * 4)).fill(-1);
    let tid;
